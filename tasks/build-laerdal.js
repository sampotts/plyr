...
// Configs
const build = require('../build-laerdal.json');
// Info from package
const minSuffix = '.min';
// Paths
...
const paths = {
  ...
  demo: {
    // Source paths
    src: {
      sass: path.join(root, 'demo-laerdal/src/sass/**/*.scss'),
      js: path.join(root, 'demo-laerdal/src/js/**/*.js'),
    },

    // Output paths
    output: path.join(root, 'demo-laerdal/dist/'),

    // Demo
    root: path.join(root, 'demo-laerdal/'),
  },
};
...
// Build distribution
gulp.task('laerdal-build', gulp.series('clean', gulp.parallel('js', 'css', 'sprites')));

// Default gulp task
gulp.task('laerdal-dev', gulp.series('laerdal-build', gulp.parallel('serve', 'watch')));
