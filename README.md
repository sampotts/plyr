...
You can also let users **upload their own caption files** at runtime, add tracks **programmatically**, and convert non-WebVTT formats (SRT,
SSA/ASS, …) using a parser of your choice. Plyr bundles no parser, so anything other than WebVTT is converted via a small `process` hook.
See [CAPTIONS.md](CAPTIONS.md) for details and a [subsrt](https://github.com/papnkukn/subsrt) example.
...
| `captions`           | Object                     | `{ active: false, language: 'auto', update: false }`                                                                           | `active`: Toggles if captions should be active by default. `language`: Sets the default language to load (if available). 'auto' uses the browser language. `update`: Listen to changes to tracks and update menu. This is needed for some streaming libraries, but can result in non-selectable language options). `upload`: Allow users to upload their own caption files, and convert non-WebVTT formats via a `process` hook. See [CAPTIONS.md](CAPTIONS.md).                                                       |
...
| `addCaptionTrack(options)`                               | Object           | Add a caption/subtitle track programmatically from inline `text` or a `src`/`url`. See [CAPTIONS.md](CAPTIONS.md).             |
